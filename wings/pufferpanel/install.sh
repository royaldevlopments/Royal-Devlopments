...
PANEL_NAME="$1"
PANEL_NAME="${PANEL_NAME:-PufferPanel}"
BINARY_NAME="${2:-pufferpanel}"
REPO="${3:-pufferpanel/pufferpanel}"
CONFIG_DIR="${4:-/etc/pufferpanel}"
SERVICE_NAME="${5:-pufferpanel}"

show_banner() {
    clear
    echo -e "${YELLOW}"
    echo -e "  ____            __  __                _  _          _ "
    echo -e " |  _ \ _ __ ___ / _|/ _| ___ _ __ ___ | || |   _ __ | |"
    echo -e " | |_) | '__/ _ \ |_| |_ / _ \ '_ \` _ \| || |_ | '_ \| |"
    echo -e " |  __/| | |  __/  _|  _|  __/ | | | | |__   _|| |_) | |"
    echo -e " |_|   |_|  \___|_| |_|  \___|_| |_| |_|  |_|(_) .__/|_|"
    echo -e "                                                 |_|     "
    echo -e "${HEADER_LINE}"
...
install_wings() {
    ...
    curl -L -o /usr/local/bin/${BINARY_NAME} \
        "https://github.com/${REPO}/releases/latest/download/pufferpanel_linux_${ARCH}"
...
LimitNOFILE=4096
ExecStart=/usr/local/bin/${BINARY_NAME} --config ${CONFIG_DIR}
Restart=on-failure
RestartSec=5
...
SERVICEEOF
    ...
    echo ""
    echo -e "  ${GOLD}Create an admin user:${NC}"
    echo -e "  ${WHITE}/usr/local/bin/pufferpanel user add${NC}"
    echo ""
    echo -e "  ${GOLD}Access the panel at:${NC} ${WHITE}http://your-server-ip:8080${NC}"
    echo -e "  ${GOLD}SFTP port:${NC} ${WHITE}5657${NC}"

    echo -e "${HEADER_LINE}"
    echo -e "\n  ${CYAN}${PANEL_NAME} DAEMON INSTALLED${NC}"
    echo -e "  ${GRAY}Check status: systemctl status ${SERVICE_NAME}${NC}"
    echo -e "  ${GRAY}Note:${NC} ${BINARY_NAME} runs both panel and daemon together${NC}"
    echo -e "${HEADER_LINE}"
}

uninstall_wings() {
    echo -e "  ${RED}WARNING: This will remove ${PANEL_NAME}!${NC}"
...
status_wings() {
    ...
    echo -e "  ${GRAY}Panel URL:${NC} http://<ip>:8080"
    echo -e "  ${GRAY}SFTP Port:${NC} 5657"
}
