...
echo -e "  ${GOLD}Pelican Domain / SSL${NC}"
echo ""
...
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    ...
    -keyout /etc/certs/pelican/privkey.pem \
    -out /etc/certs/pelican/fullchain.pem 2>/dev/null

cat > /etc/nginx/sites-available/pelican.conf << NGINX
server {
    listen 80;
    server_name ${DOMAIN};
