...
while true; do
    clear
    echo -e "${CYAN}    ____ _           _ ____       _                 ${NC}"
    echo -e "${CYAN}   / ___| |_ _ __ __| |  _ \ __ _(_)_ __           ${NC}"
    echo -e "${CYAN}  | |   | __| '__/ _\` | |_) / _\` | | '_ \          ${NC}"
    echo -e "${CYAN}  | |___| |_| | | (_| |  __/ (_| | | | | |         ${NC}"
    echo -e "${CYAN}   \____|\__|_|  \__,_|_|   \__,_|_|_| |_|         ${NC}"
    echo ""
    ...
    echo ""
    echo -ne "${BOLD}${WHITE}  ctrlpanel:~# ${NC}"
    read choice

    case $choice in
        1) run_script "panel/ctrlpanel/install.sh" ;;
        2) run_script "panel/ctrlpanel/update.sh" ;;
        3) run_script "panel/ctrlpanel/uninstall.sh" ;;
        4) run_script "panel/ctrlpanel/user.sh" ;;
        5) run_script "panel/ctrlpanel/domain.sh" ;;
