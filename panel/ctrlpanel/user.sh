...
check() {
    if [ ! -d "/var/www/ctrlpanel" ]; then echo -e "  ${RED}CtrlPanel not installed.${NC}"; return 1; fi
    cd /var/www/ctrlpanel || return 1
}

echo -e "  ${GOLD}CtrlPanel User Management${NC}"
echo ""
...
if [ "$choice" = "1" ]; then
    ...
    EMAIL="$(openssl rand -base64 4)@ctrlpanel.com"
    FIRST="$(openssl rand -base64 6)"
    LAST="$(openssl rand -base64 4)"
    php artisan make:user -n --email="${EMAIL}" --username="${USERNAME}" --password="${PASSWORD}" --admin=1 --name-first="${FIRST}" --name-last="${LAST}"
    echo ""
