...
echo -e "  ${GOLD}CtrlPanel Domain / SSL${NC}"
echo ""
...
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    ...
    -keyout /etc/certs/ctrlpanel/privkey.pem \
    -out /etc/certs/ctrlpanel/fullchain.pem 2>/dev/null

cat > /etc/nginx/sites-available/ctrlpanel.conf << NGINX
server {
    listen 80;
    server_name ${DOMAIN};
