...
check() {
    if [ ! -d "/var/www/reviactyl" ]; then echo -e "  ${RED}Reviactyl not installed.${NC}"; return 1; fi
    cd /var/www/reviactyl || return 1
}

echo -e "  ${GOLD}Reviactyl Domain / SSL${NC}"
echo ""
...
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    ...
    -keyout /etc/certs/reviactyl/privkey.pem \
    -out /etc/certs/reviactyl/fullchain.pem 2>/dev/null
