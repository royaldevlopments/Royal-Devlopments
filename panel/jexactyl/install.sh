...
download_src() {
    ...
    local LOCAL_PATH="$BASE_DIR/src/jexactyl/$SRC_FILE"
    ...
    if curl -sL "$GITHUB_RAW/src/jexactyl/$SRC_FILE" -o "$SRC_FILE" 2>/dev/null; then
...
show_banner() {
    ...
    cat << "EOF"
     __ __            _                  _
    | '__| | _____  _| |_ _ __ __ _  ___| |_ ___
    | |  | |/ _ \ \/ / __| '__/ _` |/ __| __/ __|
    | |  | |  __/>  <| |_| | | (_| | (__| |_\__ \
    |_|  |_|\___/_/\_\\__|_|  \__,_|\___|\__|___/
EOF
    echo -e "           ${WHITE}JEXACTYL INSTALLER${NC}"
    echo -e "${HEADER_LINE}"
}
...
DB_NAME=panel; DB_USER=jexactyl; DB_PASS=yourPassword
...
[ ! -f ".env.example" ] && curl -Lo .env.example https://raw.githubusercontent.com/Jexactyl/Jexactyl/develop/.env.example
cp .env.example .env
...
mkdir -p /etc/certs/jexactyl
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    -subj "/C=NA/ST=NA/L=NA/O=NA/CN=${DOMAIN}" \
    -keyout /etc/certs/jexactyl/privkey.pem -out /etc/certs/jexactyl/fullchain.pem

tee /etc/nginx/sites-available/jexactyl.conf > /dev/null << EOF
server {
    listen 80;
    server_name ${DOMAIN};
...
server {
    ...
    index index.php;
    ssl_certificate /etc/certs/jexactyl/fullchain.pem;
    ssl_certificate_key /etc/certs/jexactyl/privkey.pem;
...
tee /etc/systemd/system/jxctl.service > /dev/null << 'EOF'
[Unit]
...
echo -e "\n  ${PURPLE}Enjoy your new Jexactyl Panel!${NC}"
echo -e "${HEADER_LINE}"
