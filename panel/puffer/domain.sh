...
echo -e "  ${GOLD}PufferPanel Domain / SSL${NC}"
echo ""
...
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    ...
    -keyout /etc/certs/puffer/privkey.pem \
    -out /etc/certs/puffer/fullchain.pem 2>/dev/null

cat > /etc/nginx/sites-available/puffer.conf << NGINX
server {
    listen 80;
    server_name ${DOMAIN};
