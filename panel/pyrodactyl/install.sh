...
show_banner() {
    ...
    cat << "EOF"
              ...
              |_|    \__, |  \_/ \___|_|\_\_| |_| (_)___(_|_)
                     |___/
EOF
    echo -e "           ${WHITE}PYRODACTYL INSTALLER${NC}"
    echo -e "${HEADER_LINE}"
}
...
DB_NAME=panel; DB_USER=pyrodactyl; DB_PASS=yourPassword
...
[ ! -f ".env.example" ] && curl -Lo .env.example https://raw.githubusercontent.com/pyrodactyl-oss/pyrodactyl/develop/.env.example
cp .env.example .env
...
(crontab -l 2>/dev/null; echo "* * * * * php /var/www/pyrodactyl/artisan schedule:run >> /dev/null 2>&1") | crontab -

mkdir -p /etc/certs/pyrodactyl
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    -subj "/C=NA/ST=NA/L=NA/O=NA/CN=${DOMAIN}" \
    -keyout /etc/certs/pyrodactyl/privkey.pem -out /etc/certs/pyrodactyl/fullchain.pem

tee /etc/nginx/sites-available/pyrodactyl.conf > /dev/null << EOF
server {
    listen 80;
    server_name ${DOMAIN};
...
server {
    ...
    index index.php;
    ssl_certificate /etc/certs/pyrodactyl/fullchain.pem;
    ssl_certificate_key /etc/certs/pyrodactyl/privkey.pem;
...
ln -sf /etc/nginx/sites-available/pyrodactyl.conf /etc/nginx/sites-enabled/pyrodactyl.conf
rm -f /etc/nginx/sites-enabled/default
nginx -t && systemctl restart nginx

tee /etc/systemd/system/pyroq.service > /dev/null << 'EOF'
[Unit]
...
systemctl enable --now pyroq.service
ok "Queue running"
...
echo -e "\n  ${PURPLE}Enjoy your new Pyrodactyl Panel!${NC}"
echo -e "${HEADER_LINE}"
