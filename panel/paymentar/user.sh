...
echo -e "  ${GOLD}Paymenter User Management${NC}"
echo ""
...
if [ "$choice" = "1" ]; then
    ...
    EMAIL="$(openssl rand -base64 4)@paymenter.com"
    FIRST="$(openssl rand -base64 6)"
    LAST="$(openssl rand -base64 4)"
    php artisan app:user:create --name-first="$FIRST" --name-last="$LAST" --email="$EMAIL" --username="$USERNAME" --password="$PASSWORD" --admin=1
    echo ""
