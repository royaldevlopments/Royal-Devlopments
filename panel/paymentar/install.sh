...
download_src() {
    local SRC_FILE="paymenter.tar.gz"
    local LOCAL_PATH="$BASE_DIR/src/paymentar/$SRC_FILE"
    ...
    if curl -sL "$GITHUB_RAW/src/paymentar/$SRC_FILE" -o "$SRC_FILE" 2>/dev/null; then
